    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if academias.exists():
            ...
            if dias_restantes <= 0:
                ...
                logout(request)
                messages.error(request, 'Sua assinatura expirou! Entre em contato com o suporte.')
...
class CadastrarAluno(AcademiaView, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        academia.avaliacoes_disponiveis = academia.avaliacoes_disponiveis - int(avaliacoes)
        academia.save()
...
class RelatorioEvolucaoAcademia(TemplateView):
    def get(self, *args, **kwargs):
        ...
        if planos.count() < 2:
            messages.error(self.request, 'O Aluno não possui mais de um plano alimentar')
            return redirect('consulta_alunos')
        ...
        if not categorias_models.Antropometricos.objects.filter(user=user).exists():
            messages.error(self.request, 'O Aluno não preencheu os dados antropométricos')
            return redirect('consulta_alunos')
        dados_antropometricos = dict(categorias_models.Antropometricos.objects.filter(user=user).values()[0])
        ...
        dados_pessoais = categorias_models.DadosPessoais.objects.filter(user=user).values()[0]
